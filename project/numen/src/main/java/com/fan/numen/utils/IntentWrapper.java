...
public class IntentWrapper {
    ...
    private static List<IntentWrapper> getIntentWrapperList() {
        if (sIntentWrapperList == null) {
            sIntentWrapperList = new ArrayList<>();

            //Android 7.0+ Doze 模式
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                PowerManager pm = (PowerManager) Numen.getInstance().getContext().getSystemService(Context.POWER_SERVICE);
                boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations(Numen.getInstance().getContext().getPackageName());
                if (!ignoringBatteryOptimizations) {
                    Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                    dozeIntent.setData(Uri.parse("package:" + Numen.getInstance().getContext().getPackageName()));
            ...
            xiaomiGodIntent.putExtra("package_name", Numen.getInstance().getContext().getPackageName());
            ...
            //三星 5.0/5.1 自启动应用程序管理
            Intent samsungLIntent = Numen.getInstance().getContext().getPackageManager().getLaunchIntentForPackage("com.samsung.android.sm");
            ...
            meizuIntent.putExtra("packageName", Numen.getInstance().getContext().getPackageName());
    ...
    private static String getApplicationName() {
        if (sApplicationName == null) {
            ...
            ApplicationInfo ai;
            try {
                pm = Numen.getInstance().getContext().getPackageManager();
                ai = pm.getApplicationInfo(Numen.getInstance().getContext().getPackageName(), 0);
                sApplicationName = pm.getApplicationLabel(ai).toString();
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
                sApplicationName = Numen.getInstance().getContext().getPackageName();
            }
        }
    ...
    private boolean doesActivityExists() {
        PackageManager pm = Numen.getInstance().getContext().getPackageManager();
        List<ResolveInfo> list = pm.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
        return list != null && list.size() > 0;
    }

    private void startActivity(Activity activityContext) {
        try {
